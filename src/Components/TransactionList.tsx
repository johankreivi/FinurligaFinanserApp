...
const TransactionList: FC<ITransactionListProps> = (props) => {
    return(
        <Row data-testid="transaction-list" className="mt-5">
                ...
                <tbody data-testid="transaction-rows">
                    {
                        props.transactions.map((item, index) => 
                            <tr data-testid={`account_row_${index}`} key={`account_row_${index}`}>
                                <td data-testid={`account_row_transactionType_${index}`}>{transactionType[item.transactionType]}</td>
                                <td data-testid={`account_row_timeStamp_${index}`}>{item.timeStamp.toDateString()}</td>
                                <td data-testid={`account_row_amount_${index}`}>{item.amount.toFixed(2)} kr</td>
                                <td data-testid={`account_row_sendingAccountNumber_${index}`}>{item.sendingAccountNumber}</td>
                                ...
                                {/* <td data-testid={`account_row_accountBalance_${index}`}>{item.accountBalance}</td> */}
                            </tr>
                        )
                    }
                    <tr>
                        <td className="text-center" ><Button onClick={props.handleRedirectBack} variant="primary">Tillbaka</Button></td>
                        <td ></td>
                        <td ></td>
                        <td ></td>
                        <td ></td>
